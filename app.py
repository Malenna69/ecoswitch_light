...
try:
    import pvlib  # optionnel pour lire EPW
except Exception:
    pvlib = None
...
def UA_from_audit(a: Audit) -> float:
    ...
    em_mult = {"acier": 1.0, "alu": 0.95, "plancher": 0.85}[a.emetteurs]
    UA = a.surface_m2 * base_u * 0.5 * em_mult
...
def cop_pac(T_ext: np.ndarray, t_depart: float, isolation: str, age: int = 0) -> np.ndarray:
    ...
    cop = 3.2 + 0.06*T - 0.001*(T**2)
    cop -= 0.01*max(0.0, t_depart - 35.0)
    cop *= {"faible": 0.95, "moyenne": 1.0, "bonne": 1.03}.get(isolation, 1.0)
    cop *= max(0.0, 1.0 - 0.005*age)
...
def load_meteo(zone: str, climate_shift: float = 0.0) -> pd.DataFrame:
    ...
    if pvlib is not None and epw.exists():
        try:
            weather, _ = pvlib.iotools.read_epw(epw.as_posix())
            idx = weather.index
            ...
            if STRICT_MODE:
                raise RuntimeError(f"EPW invalid ({epw}): {e}")
    ...
    if STRICT_MODE:
        raise RuntimeError("STRICT_MODE: EPW required, fallback disabled.")
    return df
    ...
    if path.exists():
        try:
            ...
            if "price" in df.columns:
                ...
                elec = (arr[:n_hours].astype(float) / 1000.0)
                if len(elec) < n_hours:
                    elec = np.resize(elec, n_hours)
                if 0.0 < np.nanmean(elec) < 0.40:
                    return elec
        except Exception:
            pass
    h = np.arange(n_hours)
...
def _retail_vector(n_hours: int, profile: Literal["Base","HPHC","Tempo"]="Base",
                   base: float=0.1952, hp: float=0.2081, hc: float=0.1600) -> np.ndarray:
    h = np.arange(n_hours)
    if profile == "Base":
    ...
    for i in range(n_hours):
        ...
        if d=="bleu":
            ...
            v_hp = max(hp, base*1.1); v_hc = max(hc, base*0.95)
...
def load_tarifs(n_hours: int,
    ...
    gaz = np.full(n_hours, 0.11, dtype=float)
...
def _co2_vectors(n_hours: int):
    return np.full(n_hours, 0.05), np.full(n_hours, 0.227)

def simulate(audit: Audit,
             pricing_mode: Literal["wholesale","retail"]="wholesale",
             retail_profile: Literal["Base","HPHC","Tempo"]="Base",
    ...
    return {
        "cost_ref": cost_ref, "cost_pac": cost_pac, "cost_hybr": cost_hybr,
        "co2_ref": co2_ref, "co2_pac": co2_pac, "co2_hybr": co2_hybr,
        "pac_share": pac_share, "UA": UA, "price_info": price_info,
...
def log_run(audit: Audit, params: Dict[str,Any], results: Dict[str,Any]) -> None:
    ...
    payload = {
        ...
        "params": params,
        "results": {k: float(v) if isinstance(v,(int,float)) else v for k,v in results.items() if k.startswith(("cost","co2","pac_share","UA"))},
        "hashes": {
...
__all__ = ["Audit","UA_from_audit","cop_pac","eta_chaudiere","load_meteo","load_tarifs","simulate","log_run"]
